    ...
    public function process()
    {
        ...
        if (isset($Route->params->id)) {
         ...
         $filial = $AuthUser->get("office");				
			   $query = "office = '$filial' OR account_type = 'integrador' " ;	 
      ...
      //get payment
      $PayDetails = json_decode($User->get("payment_details"));
      
       $Payment = Controller::model("Payments");
       $Payment->where("is_active","=", 1)
               ->orwhere("id","=", $PayDetails[0]->paymentTerms)
				 			 ->orderBy("name","ASC")
               ->fetchData();
      ...
      $States->where("is_active","=", 1)
       ...
       $this->setVariable("States" , $States);
        ...
        }  else if (Input::post("action") == "selectFormaPagamento") {
           $this->selectFormaPagamento();
        } else if (Input::post("action") == "saveStatus") {
           $this->saveStatus();
        }
   ...
   private function saveStatus()
    {
      ...
      $Route = $this->getVariable("Route");
      $AuthUser = $this->getVariable("AuthUser");
      $idOrcamento = $Route->params->id;
     

      $orcamento = Controller::model("Order");
      $orcamento->select($idOrcamento,"id");
      $orderId = $orcamento->get("order_id");
      $orcamento->set("status",1)->save();  
      $this->logs($AuthUser->get("id"), "success","Orçamento","Orçamento Nº <b>" .$orderId ."</b> alterado o status para <b>APROVADO</b> pelo usuario  <b>" . $AuthUser->get("firstname") ."</b>");  
          
      $this->resp->result = 1;       
      $this->jsonecho();
    }
  
        private function selectFormaPagamento()
    {
      $this->resp->result = 0;
      $id = Input::post("id");
      $Cliente = Controller::model("Cliente");
      $Cliente->select($id,"id");
          
            $Pagamentos = Controller::model("Payments");
            $Pagamentos->where("is_active","=",1)
                       ->orderBy("name","DESC")
                       ->fetchData();

          
        $arrayPagamento = json_decode($Cliente->get("p_pagamentos"),true);
          
        foreach($Pagamentos->getDataAs("Payment") as $p){
              $id = $p->get("id");
              if($arrayPagamento != null){
                
               if (in_array($id, $arrayPagamento)) {
			...
			$this->resp->pagamento = $arrayPERMISSOES;     
        $this->resp->result = 1;       
    ...
    private function selectClientes()
    {
     ...
     $idResponsavel = $AuthUser->get("id"); 
      $pesquisa = Input::post("q");
   if($AuthUser->get("account_type") == "representante"){
          $Permissao = json_decode($AuthUser->get("estados_atuacao"), true);
          $Permissao = '("' . implode('","', $Permissao) . '")';
     
          $query = " uf IN $Permissao AND owner = '$idResponsavel' AND name LIKE '%$pesquisa%'";
		
	   $Clientes = Controller::model("Clientes");
     $Clientes->where(DB::raw("$query"))						
			        ->orderBy("name","ASC")
              ->fetchData();
      } else {
