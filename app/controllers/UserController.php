...
class UserController extends Controller
{
    ...
    public function process()
    {
       ...
       if($User->get('account_type') == "diretoria" && $AuthUser->get('account_type') != "diretoria"){
           header("Location: ".APPURL."/users");
             exit;
       }
...
// 						if($AuthUser->get('account_type') != 'gerente' || $AuthUser->get('account_type') != 'supervisor'){
// // 						  header("Location: ".APPURL."/users");
// //               exit;
// 						}
       ...
       $Estados->orderBy("id","DESC")
             ...
             ->fetchData();
				...
				foreach($Estados->getDataAs("State") as $estado){
					 $ArrayEstados[] =  array( 
							"uf" => strtoupper($estado->get("uf")),
							"nome" => $estado->get("name")
					 ); 
				}
			
			  $this->setVariable("ArrayEstados", $ArrayEstados);		
       ...
       $Users->orderBy("id","DESC")
			 ...
			 $Filial->orderBy("id","DESC")
			  ...
			  if($AuthUser->get('account_type') == "supervisor" || $AuthUser->get('account_type') == "representante" ){
...
// 			  $filiais = json_encode($permissoes);
      ...
      $encodeFilialSelecionada = "";
			if(Input::post("filialSelecionada") != null){
			$encodeFilialSelecionada = json_encode(Input::post("filialSelecionada"), true);
			}
      $encodeEstadoSelecionado = "";
      if(Input::post("estadoSelecionado") != null){
			$encodeEstadoSelecionado = json_encode(Input::post("estadoSelecionado"), true);
			}
            ...
            if (Input::post("account_type") == "integrador") {
              ...
              $User->set("firstname", Input::post("firstname"))
                   ...
                   if($AuthUser->isMaster()){
								     $User->set("estados_atuacao", $encodeEstadoSelecionado);
                   }
