...
class WebhookController extends Controller
{
    ...
    public function process()
    {
      ...
      $event = json_decode($data, true);    
			...
			$telefone = strval($event['entry'][0]['changes'][0]['value']['messages'][0]['from']);
      ...
      $data_formatada =  $date->format('Y-m-d H:i:s');
			...
			$Webhook->set("nome", $nome)
							->set("telefone", $telefone)
							->set("mensagem", $mensagem)
							->set("data_envio", $data_formatada);
			
			// Histórico Mensagem			
			$UltimaMsg = Controller::model("Webhooks");
			$UltimaMsg->where("telefone", "=", $telefone)
								->where("data_envio", ">=", $DataPadrao)
								->orderBy("id", "DESC")
								->limit(1)
								->fetchData();
			
			if($UltimaMsg->getTotalCount() < 1) {	
				sendWhatsappInicioConversa($telefone, "cv_inicio", $nome);
				$protocolo = uniqid();
				$Webhook->set("protocolo", $protocolo);
				sendWhatsappCustomizada($telefone, "*Protocolo de Atendimento:* " . $protocolo);
				$mensagem = "menu";
			} else {
				foreach($UltimaMsg->getDataAs("Webhook") as $w){
					$Webhook->set("protocolo", $w->get("protocolo"));
				}
			}
			
			$ModeloMensagem = $this->consultaMensagem($mensagem);
			
			// Dispara mensagem			
			try {
				$this->modeloMensagem($ModeloMensagem, $telefone);	
				$Webhook->set("status", 1);
			} catch (Exception $e){
				$Webhook->set("status", 0);
			}
...
// 	
	...
	private function consultaMensagem($msg)
	{
		$RespostaMsg = Controller::model("OpcoesMensagens");
		$RespostaMsg->where("palavra", "LIKE", "%" .$msg . "%")
								->limit(1)
								->fetchData();
		
		$Resposta = "Desculpe mas não entendi o que você falou.";
		
		foreach($RespostaMsg->getDataAs("OpcaoMensagens") as $m){
	...
	private function modeloMensagem($Mensagem, $telefoneEnvio)
	{
		if ($Mensagem == "menu_opcoes"){
			sendWhatsappPadrao($telefoneEnvio,"menu_opcoes");
		} else {
			sendWhatsappCustomizada($telefoneEnvio,$Mensagem);	
		}				
