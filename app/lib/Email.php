...
class Email extends PHPMailer{
    ...
    public static function sendNotification($type = "nova-cotacao", $data = [])
    {
        switch ($type) { 
            ...
            case "senha-alterada":
                ...
                break; 
            case "solicitacao-comissao":
                return self::SendSolicitacaoComissao($data);
                break;
            case "liberado-comissao":
                return self::SendComissaoLiberada($data);
                break;
            case "pago-comissao":
                return self::SendComissaoPaga($data);
                break;
  ...
  /**
    ...
    {       
        ...
        $mail->addAddress('no-reply@horus.com.br', 'Conferência ');    
		$mail->addAddress('rainey.soares@horus.com.br', 'Rainey');   
        $mail->isHTML(true);
        $emailbody = "<p>Caro(a) usuário(a), </p>"
  ...
  /**
    ...
    {       
      ...
      private static function SendSolicitacaoComissao($data = [])
    {       
        $mail = new PHPMailer();
        $mail = self::configSmtp($mail);        
      
        $mail->Subject = "Solicitação de comissionamento!";
      
        $mail->addAddress('gustavo.viana@horus.com.br', 'Gustavo ');
        $mail->addAddress('suporte@horus.com.br', 'Conferência ');
        $mail->addAddress('sabrinna@horus.com.br', 'Sabrinna ');
        //$mail->addAddress('dhiego.lemes@horus.com.br', 'Conferência ');
      
        $mail->isHTML(true);
        $emailbody = "<p><b> Foi solicitado comissionamento para os dados abaixo:</b> </p>"
                
                   . "<p><b>ID Santri: </b> ". $data['id_santri'] ."</p>"
                   . "<p><b>Razão Social: </b> ".$data['razao'] ."</p>"
                   . "<p><b>CNPJ: </b> ". formata_cpf_cnpj($data['cnpj']) ." </p>"
              
                   . "<a href='".APPURL."/comissionamentos/".$data['id'] ."' target='_blank'>Acesse agora o site.</a></p>"
                    
                   . "<p>E-Mail enviado automaticamente via Painel Horus Solar.</p>"
                   . "<p>Não utilizar a opção de 'Responder ao Remetente', pois sua mensagem não será lida.</p>"
                   . "<p>Painel Solar - Horus S/A</p>";
   ...
   private static function SendComissaoLiberada($data = [])
      {       
        $mail = new PHPMailer();
        $mail = self::configSmtp($mail);        
      
        $mail->Subject = "Comissionamento Liberado Controladoria!";
      
        $mail->addAddress('gustavo.viana@horus.com.br', 'Gustavo ');
        $mail->addAddress('suporte@horus.com.br', 'Conferência ');
        $mail->addAddress('leticia.lima@horus.com.br', 'Leticia ');
        //$mail->addAddress('no-reply@horus.com.br', 'Conferência ');
        //$mail->addAddress('dhiego.lemes@horus.com.br', 'Conferência ');
      
        $mail->isHTML(true);
        $emailbody = "<p><b>Comissionamento liberado para pagamento, segue os dados abaixo:</b> </p>"
     ...
     private static function SendComissaoPaga($data = [])
      {       
        $mail = new PHPMailer();
        $mail = self::configSmtp($mail);        
      
        $mail->Subject = "Comissionamento Pago!";
        ...
        $mail->isHTML(true);
        $emailbody = "<p><b>Comissionamento pago para os dados abaixo:</b> </p>"
                
                   . "<p><b>ID Santri: </b> ". $data['id_santri'] ."</p>"
                   . "<p><b>Razão Social: </b> ".$data['razao'] ."</p>"
                   . "<p><b>CNPJ: </b> ". formata_cpf_cnpj($data['cnpj']) ." </p>"
                   . "<p><b>Dados Pagamento: </b> ".$data['pagamento'] ."</p>"
