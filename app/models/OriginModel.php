<?php
	...
	class OriginModel extends DataEntry
	{
		/**
	    ...
	    public function select($uniqid)
	    {
	    	...
	    	if ($col) {
		    	$query = DB::table(TABLE_PREFIX.TABLE_ORIGINS)
			    	      ->where($col, "=", $uniqid)
			    	      ->limit(1)
			    	      ->select("*");
		    	if ($query->count() == 1) {
	    ...
	    public function insert()
	    {
	    	...
	    	$id = DB::table(TABLE_PREFIX.TABLE_ORIGINS)
		    	->insert(array(
		    		"id" => null,
		    		"name" => $this->get("name"),
		    		"is_active" => $this->get("is_active"),
		    		"owner" => $this->get("owner"),
		    		"cod_origin" => $this->get("cod_origin"),
		    		"date" => $this->get("date"),
		    	));
	    ...
	    public function delete()
	    {
	    	...
	    	DB::table(TABLE_PREFIX.TABLE_ORIGINS)->where("id", "=", $this->get("id"))->delete();
	    	$this->is_available = false;
	    	return true;
	    }
