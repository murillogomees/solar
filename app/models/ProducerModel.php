<?php
	...
	class ProducerModel extends DataEntry
	{
		/**
	    ...
	    public function select($uniqid)
	    {
	    	...
	    	if ($col) {
		    	$query = DB::table(TABLE_PREFIX.TABLE_PRODUCERS)
			    	      ->where($col, "=", $uniqid)
			    	      ->limit(1)
			    	      ->select("*");
		    	if ($query->count() == 1) {
	    ...
	    public function insert()
	    {
	    	...
	    	$id = DB::table(TABLE_PREFIX.TABLE_PRODUCERS)
		    	->insert(array(
		    		"id" => null,
		    		"name" => $this->get("name"),
	    ...
	    public function delete()
	    {
	    	...
	    	DB::table(TABLE_PREFIX.TABLE_PRODUCERS)->where("id", "=", $this->get("id"))->delete();
	    	$this->is_available = false;
	    	return true;
	    }
