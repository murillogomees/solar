 ...
 *
 */
class ShippingModel extends DataList
{
	/**
	 * Initialize
	 */
	...
	{
		$this->setQuery(DB::table(TABLE_PREFIX.TABLE_SHIPPING));
	}
    ...
    public function search($search_query)
    {
        ...
        foreach ($search_strings as $sq) {
            ...
            $query->where(function($q) use($sq) {
                $q->where(TABLE_PREFIX.TABLE_SHIPPING.".name", "LIKE", $sq."%");
            });
        }
    ...
    public function fetchData()
    {
        ...
        $this->getQuery()
             ->select(TABLE_PREFIX.TABLE_SHIPPING.".*");
