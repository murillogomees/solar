...
<html lang="<?= ACTIVE_LANG ?>">
    <head>
        ...
        <title><?= __("Clientes - ". site_settings("site_name")) ?></title>
    </head>

    <body>
        <?php
            $Nav = new stdClass;
            $Nav->activeMenu = "clients";
            require_once(APPPATH.'/views/fragments/navigation.fragment.php');
        ?>

        <?php
            $TopBar = new stdClass;
            $TopBar->title = __("Clientes");
            $TopBar->btn = array(
                "icon" => "sli sli-user-follow",
                "title" => __("Adicionar Cliente"),
                "link" => APPURL."/clients/new"
            );
            require_once(APPPATH.'/views/fragments/topbar.fragment.php');
        ?>

        <?php require_once(APPPATH.'/views/fragments/clientes.fragment.php'); ?>
