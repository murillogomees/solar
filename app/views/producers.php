...
<html lang="<?= ACTIVE_LANG ?>">
    <head>
        ...
        <title><?= __("Fabricantes - ". site_settings("site_name")) ?></title>
    </head>
    ...
    <body>
        ...
        <?php
            $TopBar = new stdClass;
            $TopBar->title = __("Fabricantes");
            $TopBar->btn = array(
                "icon" => "sli sli-layers",
                "title" => __("Adicionar Fabricante"),
                "link" => APPURL."/producers/new"
            );
            require_once(APPPATH.'/views/fragments/topbar.fragment.php');
        ?>

        <?php require_once(APPPATH.'/views/fragments/producers.fragment.php'); ?>
