...
<html lang="<?= ACTIVE_LANG ?>">
    <head>
        ...
        <title><?= __("Kits de Produto - ". site_settings("site_name")) ?></title>
    </head>
    ...
    <body>
        ...
        <?php
            $TopBar = new stdClass;
            $TopBar->title = __("Kits de Produto");
            $TopBar->btn = array(
                "icon" => "sli sli-directions",
                "title" => __("Adicionar Kit de Produto"),
                "link" => APPURL."/product-kits/new"
            );
            require_once(APPPATH.'/views/fragments/topbar.fragment.php');
        ?>

        <?php require_once(APPPATH.'/views/fragments/product-kits.fragment.php'); ?>
