...
<html lang="<?= ACTIVE_LANG ?>">
    <head>
        ...
        <title><?= __("Segmentos do Produto - ". site_settings("site_name")) ?></title>
    </head>
    ...
    <body>
        ...
        <?php
            $TopBar = new stdClass;
            $TopBar->title = __("Segmentos do Produto");
            $TopBar->btn = array(
                "icon" => "sli sli-directions",
                "title" => __("Adicionar Segmento"),
                "link" => APPURL."/product-segments/new"
            );
            require_once(APPPATH.'/views/fragments/topbar.fragment.php');
        ?>

        <?php require_once(APPPATH.'/views/fragments/product-segments.fragment.php'); ?>
