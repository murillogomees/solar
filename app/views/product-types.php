...
<html lang="<?= ACTIVE_LANG ?>">
    <head>
        ...
        <title><?= __("Tipo de Produto - ". site_settings("site_name")) ?></title>
    </head>
    ...
    <body>
        ...
        <?php
            $TopBar = new stdClass;
            $TopBar->title = __("Tipos do Produto");
            $TopBar->btn = array(
                "icon" => "sli sli-wrench",
                "title" => __("Adicionar Tipo de Produto"),
                "link" => APPURL."/product-types/new"
            );
            require_once(APPPATH.'/views/fragments/topbar.fragment.php');
        ?>

        <?php require_once(APPPATH.'/views/fragments/product-types.fragment.php'); ?>
