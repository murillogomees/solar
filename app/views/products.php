...
<html lang="<?= ACTIVE_LANG ?>">
    <head>
        ...
        <title><?= __("Produtos - ". site_settings("site_name")) ?></title>
    </head>

    <body>
        <?php
            $Nav = new stdClass;
            $Nav->activeMenu = "products";
            require_once(APPPATH.'/views/fragments/navigation.fragment.php');
        ?>

        <?php
            $TopBar = new stdClass;
            $TopBar->title = __("Produtos");
            $TopBar->btn = array(
                "icon" => "sli sli-social-dropbox",
                "title" => __("Adicionar Produto"),
                "link" => APPURL."/products/new"
            );
            require_once(APPPATH.'/views/fragments/topbar.fragment.php');
        ?>

        <?php require_once(APPPATH.'/views/fragments/products.fragment.php'); ?>      
