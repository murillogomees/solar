...
<html lang="<?= ACTIVE_LANG ?>">
    <head>
        ...
        <title><?= __("Perfil Tributário ". site_settings("site_name")) ?></title>
    </head>
    ...
    <body>
        ...
        <?php
            $TopBar = new stdClass;
            $TopBar->title = __("Perfil Tributário");
            $TopBar->btn = array(
                "icon" => "sli sli-user-follow",
                "title" => __("Adicionar Perfil Tributário"),
                "link" => APPURL."/taxprofiles/new"
            );
            require_once(APPPATH.'/views/fragments/topbar.fragment.php');
        ?>

        <?php require_once(APPPATH.'/views/fragments/tax-profiles.fragment.php'); ?>
